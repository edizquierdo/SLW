...
// Fitness function
// ------------------------------------
double AsymptoticFitness(TVector<double> &genotype, std::string oFileName)
{
	...
	outputfile << Insect.Leg.Angle << "," << Insect.Leg.Omega << "," << Insect.NervousSystem.NeuronOutput(1) << "," << Insect.NervousSystem.NeuronOutput(2) << endl;
	while (! ((newFootState < 0.5) && (pastFootState > 0.5)))
	{
		Insect.Step1CPG(StepSize);
		outputfile << Insect.Leg.Angle << "," << Insect.Leg.Omega << "," << Insect.NervousSystem.NeuronOutput(1) << "," << Insect.NervousSystem.NeuronOutput(2) << endl;
...
int main (int argc, const char* argv[])
{
	...
	std::string const & oFileName = "beh.csv"; //argv[2];
	genefile.open(gFileName);
	TVector<double> genotype(1, VectSize);
	genefile >> genotype;
	AsymptoticFitness(genotype,oFileName);
	genefile.close();
	return 0;
}
